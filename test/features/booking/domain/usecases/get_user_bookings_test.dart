...
import 'package:goldengoose/features/booking/domain/use_cases/get_user_bookings.dart';
...
void main() {
  ...
  late GetUserBookings usecase;

  setUp(() {
    mockRepo = MockBookingRepository();
    usecase = GetUserBookings(mockRepo);
  });
  ...
  group('GetUserBookings', () {
    ...
    test('calls repository.getUserBookings', () async {
      when(() => mockRepo.getUserBookings('u1')).thenAnswer((_) async => bookings);
      final result = await usecase('u1');
      expect(result, bookings);
      verify(() => mockRepo.getUserBookings('u1')).called(1);
    });

    test('propagates errors from repository', () async {
      when(() => mockRepo.getUserBookings('u1')).thenThrow(Exception('fail'));
